...
#!/bin/sh
git daemon --base-path=/git-data --port=2222 --export-all --verbose --enable=receive-pack
EOF
...
# install git and prepare dirs
RUN apk add --no-cache git git-daemon
RUN git init --bare /git-data/${GIT_REPO}.git
...
RUN adduser \
    ...
    --uid "${UID}" \
    git
RUN chown -R git:git git-data
USER git

# Copy the executable from the "build" stage.
COPY --from=build /bin/entrypoint.sh /bin/
...
HEALTHCHECK CMD ps | grep git-daemon
